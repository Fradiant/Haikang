<template>
  <div class="teamInfoPages">
    <div class="card-container">
      <el-card v-for="(item, index) in tableData" :key="index" class="custom-card" @click="shishiClick">
        <template v-for="(item1, index1) in tableHeaders">
          <template slot="header" v-if="item1.title === true && item1.key === 'ScheduleName'">
            <span>{{item[item1.key]}}</span>
          </template>
          <template slot="header" v-else-if="item1.titleTag === true && item1.key === 'ScheduleStatus'">
            <el-tag type="success">{{tagStatus(item[item1.key])}}</el-tag>
          </template>
          <div v-else class="table-cell ellipsis">
            <span class="table-header" v-if="item1.formatter && item1.key === 'PersonList'">
            ...
            <div class="patrolPointItem" v-else-if="item1.key === 'what' && item[item1.key]">
